...
SAFE_BROWSING_API_KEY = os.getenv('SAFE_BROWSING_API_KEY')  # اختياري لكن مستحسن
...
class TTLCache:
    ...
    def cleanup(self):
        ...
        for k in list(self._data.keys()):
            if now - self._data[k][1] > self._ttl:
                del self._data[k]
...
class DeepGuard(commands.Bot):
    ...
    async def setup_hook(self):
        # اقتصاد في الموارد: IPv4 فقط، كاش DNS، حدود اتصال
        self.http_session = aiohttp.ClientSession(
            connector=aiohttp.TCPConnector(
                limit=20,
                ttl_dns_cache=300,
                use_dns_cache=True,
                family=2  # AF_INET = IPv4 فقط
            ),
    ...
    def load_badwords(self):
        try:
            ...
            print(f"[DeepGuard] {len(self.badwords_patterns)} درع حماية محمل.")
        except FileNotFoundError:
            with open('badwords.json', 'w', encoding='utf-8') as f:
                json.dump([], f)
            print("[DeepGuard] تم إنشاء badwords.json فارغ.")
...
async def get_log_thread(guild: discord.Guild):
    if hasattr(guild, 'get_channel_or_thread'):
        return guild.get_channel_or_thread(LOG_CHANNEL_ID)
...
async def send_log(guild: discord.Guild, title: str, desc: str, color: discord.Color,
    ...
    log_ch = await get_log_thread(guild)
    if not log_ch:
        print(f"[LOG ERROR] الثريد {LOG_CHANNEL_ID} غير موجود")
        return
...
async def check_url_api(url: str) -> tuple[bool, str]:
    ...
    if SAFE_BROWSING_API_KEY and bot.http_session:
        try:
            ...
            async with bot.http_session.post(api_url, json=payload) as resp:
                if resp.status == 200:
...
async def auto_punish(member: discord.Member, reason: str, severity: str = "low",
    ...
    now = datetime.utcnow()
    msg_link = msg.jump_url if (msg and msg.id) else None
    ...
    if severity == "critical":
        ...
        if warns < 3:
            ...
            if channel:
                try:
                    await channel.send(warn_text, delete_after=15)
                except:
                    pass
            try:
    ...
    try:
        ...
        if channel:
            try:
                await channel.send(f"🚫 **{member.mention} تايم أوت**\n**السبب:** {punish_reason}\n**المدة:** {duration}",
                                  delete_after=25)
            except:
                pass
        try:
...
async def on_ready():
    await bot.tree.sync()
    print(f'[DeepGuard] Logged in as {bot.user} (ID: {bot.user.id})')
...
async def on_message(message: discord.Message):
    ...
    # حماية ثريد اللوقز: حذف أي رسالة + إشعار
    if message.channel.id == LOG_CHANNEL_ID:
        try:
            await message.delete()
            await message.author.send("🚫 **ثريد السجلات للقراءة فقط.** لا يمكنك التحدث هنا.",
                                      delete_after=15)
        except:
            pass
        return
    ...
    try:
        ...
        if urls:
            for url in urls:
                is_bad, reason = await check_url_api(url)
                if is_bad:
                    await message.delete()
                    await auto_punish(author, f"رابط مشبوه/خبيث: {reason}", "high", channel, message)
                    return
                elif any(s in url.lower() for s in SHORTENER_DOMAINS):
                    await send_log(message.guild, "تنبيه: رابط مختصر",
                                  f"رابط يحتاج مراجعة يدوية: `{url}`",
                                  discord.Color.teal(), author, message.jump_url, channel)
...
async def timeout_cmd(interaction: discord.Interaction, member: discord.Member, minutes: int, reason: str = "بدون سبب"):
    ...
    try:
        ...
        await interaction.response.send_message("❌ البوت لا يملك صلاحية كافية (تأكد من ترتيب الرتبة).", ephemeral=True)
